      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          animatedValues.forEach((stat, index) => {
            ...
            const interval = setInterval(countUp, 100); // Adjust the interval for speed
    ...
    <div ref={statsRef} className="flex flex-wrap justify-around p-8 bg-none">
      {animatedValues.map((stat, index) => (
        ...
        >
          ...
          <div className="flex">
            <span className="text-[88px] mr-4 flex mt-6">{stat.icon}</span>
            <div>
              <h2 className="flex justify-center text-5xl font-extrabold">
                {stat.count}
              </h2>
